...
import grpc
from nietzsche_legacy import nietzsche_db_pb2, nietzsche_db_pb2_grpc

def test_replication():
    # Connect to Leader
    leader_channel = grpc.insecure_channel('localhost:50051')
    leader_stub = nietzsche_db_pb2_grpc.DatabaseStub(leader_channel)
    
    # Insert data
    metadata = [('x-api-key', 'I_LOVE_NIETZSCHEDB')]
    ...
    # Query Leader digest
    digest_req = nietzsche_db_pb2.DigestRequest(collection='default')
    ...
    follower_stub = nietzsche_db_pb2_grpc.DatabaseStub(follower_channel)
