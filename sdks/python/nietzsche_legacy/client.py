...
from .proto import nietzsche_db_pb2
from .proto import nietzsche_db_pb2_grpc
...
class NietzscheBaseClient:
    def __init__(self, host: str = "localhost:50051", api_key: Optional[str] = None, embedder: Optional[BaseEmbedder] = None, user_id: Optional[str] = None):
        ...
        if user_id:
            meta.append(('x-nietzsche-user-id', user_id))
    ...
    def create_collection(self, name: str, dimension: int, metric: str) -> bool:
        req = nietzsche_db_pb2.CreateCollectionRequest(name=name, dimension=dimension, metric=metric)
        try:
    ...
    def delete_collection(self, name: str) -> bool:
        req = nietzsche_db_pb2.DeleteCollectionRequest(name=name)
        try:
    ...
    def get_collection_stats(self, name: str) -> Dict:
        req = nietzsche_db_pb2.CollectionStatsRequest(name=name)
        try:
    ...
    def insert(self, id: int, vector: List[float] = None, document: str = None, metadata: Dict[str, str] = None, collection: str = "", durability: int = Durability.DEFAULT) -> bool:
        ...
        req = nietzsche_db_pb2.InsertRequest(
            id=id,
            vector=vector,
    ...
    def batch_insert(self, vectors: List[List[float]], ids: List[int], metadatas: List[Dict[str, str]] = None, collection: str = "", durability: int = Durability.DEFAULT) -> bool:
        ...
        if metadatas is None:
            for v, i in zip(vectors, ids):
                proto_vectors.append(nietzsche_db_pb2.VectorData(
        ...
        req = nietzsche_db_pb2.BatchInsertRequest(
            collection=collection,
    ...
    def search(self, vector: List[float] = None, query_text: str = None, top_k: int = 10, filter: Dict[str, str] = None, filters: List[Dict] = None, hybrid_query: str = None, hybrid_alpha: float = None, collection: str = "") -> List[Dict]:
        ...
        if filters:
            for f in filters:
                if f.get("type") == "match":
                    proto_filters.append(nietzsche_db_pb2.Filter(
                        match=nietzsche_db_pb2.Match(key=f["key"], value=f["value"])
                    ))
                    ...
                    proto_filters.append(nietzsche_db_pb2.Filter(
                        range=nietzsche_db_pb2.Range(**kwargs)
                    ))
    ...
    def search_batch(
        ...
        for vector in vectors:
            searches.append(
                nietzsche_db_pb2.SearchRequest(
                    vector=self._normalize_vector(vector),
                    top_k=top_k,
                    collection=collection,
                )
            )
        req = nietzsche_db_pb2.BatchSearchRequest(searches=searches)
        try:
    ...
    def trigger_vacuum(self) -> bool:
        try:
            self.stub.TriggerVacuum(nietzsche_db_pb2.Empty(), metadata=self.metadata)
            return True
        except grpc.RpcError as e:
            print(f"RPC Error: {e}")
            return False
    ...
    def rebuild_index(self, collection: str) -> bool:
        req = nietzsche_db_pb2.RebuildIndexRequest(name=collection)
        try:
    ...
    def trigger_snapshot(self) -> bool:
        try:
            resp = self.stub.TriggerSnapshot(nietzsche_db_pb2.Empty(), metadata=self.metadata)
            return True
        except grpc.RpcError as e:
            print(f"RPC Error: {e}")
            return False
    ...
    def configure(self, ef_search: int = None, ef_construction: int = None, collection: str = "") -> bool:
        req = nietzsche_db_pb2.ConfigUpdate(collection=collection)
