...
use nietzsche_core::SearchParams;
...
pub async fn start_http_server(
    ...
    let api_key_hash = std::env::var("NDB_API_KEY").ok().map(|key| {
        let mut hasher = Sha256::new();
...
async fn get_status(
    ...
    let dim = std::env::var("NDB_DIMENSION").unwrap_or("1024".to_string());
    let metric = std::env::var("NDB_METRIC").unwrap_or("l2".to_string());
    let quantization = std::env::var("NDB_QUANTIZATION_LEVEL").unwrap_or("scalar".to_string());
...
async fn get_prometheus_metrics(
    ...
    let body = format!(
        "# HELP nietzsche_active_collections Number of collections in memory\n\
         # TYPE nietzsche_active_collections gauge\n\
         nietzsche_active_collections {active}\n\
         # HELP nietzsche_idle_collections Number of collections unloaded to disk\n\
         # TYPE nietzsche_idle_collections gauge\n\
         nietzsche_idle_collections {idle}\n\
         # HELP nietzsche_total_vectors Total number of vectors in active collections\n\
         # TYPE nietzsche_total_vectors gauge\n\
         nietzsche_total_vectors {total_vecs}\n\
         # HELP nietzsche_ram_usage_mb Memory usage in MB\n\
         # TYPE nietzsche_ram_usage_mb gauge\n\
         nietzsche_ram_usage_mb {ram_mb}\n\
         # HELP nietzsche_disk_usage_mb Disk usage in MB\n\
         # TYPE nietzsche_disk_usage_mb gauge\n\
         nietzsche_disk_usage_mb {disk_mb}\n\
         # HELP nietzsche_cpu_usage_percent CPU usage percent\n\
         # TYPE nietzsche_cpu_usage_percent gauge\n\
         nietzsche_cpu_usage_percent {cpu_percent}\n\
         # HELP nietzsche_search_total Total search operations via REST API\n\
         # TYPE nietzsche_search_total counter\n\
         nietzsche_search_total {search_count}\n\
         # HELP nietzsche_search_latency_avg_us Average search latency in microseconds\n\
         # TYPE nietzsche_search_latency_avg_us gauge\n\
         nietzsche_search_latency_avg_us {avg_search_us}\n\
         # HELP nietzsche_insert_total Total insert operations via REST API\n\
         # TYPE nietzsche_insert_total counter\n\
         nietzsche_insert_total {insert_count}\n\
         # HELP nietzsche_insert_latency_avg_us Average insert latency in microseconds\n\
         # TYPE nietzsche_insert_latency_avg_us gauge\n\
         nietzsche_insert_latency_avg_us {avg_insert_us}\n\
         # HELP nietzsche_delete_total Total delete operations via REST API\n\
         # TYPE nietzsche_delete_total counter\n\
         nietzsche_delete_total {delete_count}\n\
         # HELP nietzsche_errors_total Total error responses via REST API\n\
         # TYPE nietzsche_errors_total counter\n\
         nietzsche_errors_total {error_count}\n"
    );
...
async fn get_logs() -> Json<Vec<String>> {
    Json(vec![
        "[SYSTEM] NietzscheDB v2.0.0 Online".into(),
