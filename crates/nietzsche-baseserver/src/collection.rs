...
use dashmap::DashMap;
use nietzsche_core::{Collection, FilterExpr, GlobalConfig, Metric, SearchParams, SearchResult};
use nietzsche_hnsw::HnswIndex;
use nietzsche_proto::db::{replication_log, InsertOp, ReplicationLog};
use nietzsche_vecstore::{wal::Wal, VectorStore};
...
pub struct CollectionImpl<const N: usize, M: Metric<N>> {
    ...
    data_dir: PathBuf,
    // Quantization Mode
    mode: nietzsche_core::QuantizationMode,
...
impl<const N: usize, M: Metric<N>> CollectionImpl<N, M> {
    ...
    pub async fn new(
        ...
        let ef_cons_env = std::env::var("NDB_HNSW_EF_CONSTRUCT")
            .unwrap_or("100".to_string())
            .parse()
            .unwrap_or(100);
        let ef_search_env = std::env::var("NDB_HNSW_EF_SEARCH")
            .unwrap_or("10".to_string())
            .parse()
            .unwrap_or(10);
        let m_env = std::env::var("NDB_HNSW_M")
        ...
        let element_size = match mode {
            nietzsche_core::QuantizationMode::ScalarI8 => {
                nietzsche_core::vector::QuantizedHyperVector::<N>::SIZE
            }
            nietzsche_core::QuantizationMode::Binary => {
                nietzsche_core::vector::BinaryHyperVector::<N>::SIZE
            }
            nietzsche_core::QuantizationMode::None => {
                nietzsche_core::vector::HyperVector::<N>::SIZE
            }
        };
        ...
        // WAL
        let sync_mode_str = std::env::var("NDB_WAL_SYNC_MODE")
            .unwrap_or_else(|_| "async".to_string())
            .to_lowercase();

        let sync_mode = match sync_mode_str.as_str() {
            "strict" | "fsync" => nietzsche_vecstore::wal::WalSyncMode::Strict,
            "batch" => nietzsche_vecstore::wal::WalSyncMode::Batch,
            _ => nietzsche_vecstore::wal::WalSyncMode::Async,
        };

        if sync_mode == nietzsche_vecstore::wal::WalSyncMode::Strict {
            println!("🔒 WAL Durability: STRICT (fsync on every write)");
        } else if sync_mode == nietzsche_vecstore::wal::WalSyncMode::Batch {
        ...
        Wal::replay(&wal_path_clone, |entry| {
            let nietzsche_vecstore::wal::WalEntry::Insert {
                id,
                vector,
                metadata,
                logical_clock,
        ...
        let concurrency_env = std::env::var("NDB_INDEXER_CONCURRENCY")
        ...
        let search_concurrency_env = std::env::var("NDB_SEARCH_CONCURRENCY")
        ...
        let snap_interval = std::env::var("NDB_SNAPSHOT_INTERVAL_SEC")
...
impl<const N: usize, M: Metric<N>> Collection for CollectionImpl<N, M> {
    ...
    async fn insert(
        ...
        clock: u64,
        durability: nietzsche_core::Durability,
    ) -> Result<(), String> {
        if vector.len() != N {
            return Err(format!(
...

        {
            ...
            if durability == nietzsche_core::Durability::Strict {
    ...
    fn quantization_mode(&self) -> nietzsche_core::QuantizationMode {
        self.mode
    }
    ...
    async fn optimize_with_filter(
        &self,
        filter: Option<nietzsche_core::VacuumFilterQuery>,
    ) -> Result<(), String> {
        if filter.is_none() {
                ...
                .filter(|(_id, _vec, meta)| {
                    if let Some(val_str) = meta.get(&filter.key) {
                        if let Ok(val) = val_str.parse::<f64>() {
                            match filter.op {
                                nietzsche_core::VacuumFilterOp::Lt => val < filter.value,
                                nietzsche_core::VacuumFilterOp::Lte => val <= filter.value,
                                nietzsche_core::VacuumFilterOp::Gt => val > filter.value,
                                nietzsche_core::VacuumFilterOp::Gte => val >= filter.value,
                                nietzsche_core::VacuumFilterOp::Eq => (val - filter.value).abs() < 1e-9,
                                nietzsche_core::VacuumFilterOp::Ne => (val - filter.value).abs() >= 1e-9,
                            }
                        } else {
