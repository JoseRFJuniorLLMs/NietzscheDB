...
#[cfg(feature = "embed")]
use nietzsche_embed::{ApiProvider, Metric, OnnxVectorizer, RemoteVectorizer, Vectorizer};
use nietzsche_proto::db::database_server::{Database, DatabaseServer};
use nietzsche_proto::db::{replication_log, Empty, ReplicationLog};
use nietzsche_proto::db::{
    metadata_value, BatchInsertRequest, BatchSearchRequest, BatchSearchResponse,
    CollectionStatsRequest, CollectionStatsResponse, ConfigUpdate,
    ...
    ListCollectionsResponse, MetadataValue, MonitorRequest, SearchRequest, SearchResponse,
    SearchResult, SystemStats, TraverseRequest, TraverseResponse, VectorDeletedEvent,
    VectorInsertedEvent,
};
...
fn default_ef_search() -> usize {
    ...
    *DEFAULT_EF_SEARCH.get_or_init(|| {
        std::env::var("NDB_HNSW_EF_SEARCH")
...
fn range_bounds_f64(r: &nietzsche_proto::db::Range) -> (Option<f64>, Option<f64>) {
...
fn build_filters(
    ...
    Vec<nietzsche_core::FilterExpr>,
    nietzsche_core::SearchParams,
) {
    ...
    for f in req.filters {
        if let Some(cond) = f.condition {
            match cond {
                nietzsche_proto::db::filter::Condition::Match(m) => {
                    complex_filters.push(nietzsche_core::FilterExpr::Match {
                        key: m.key,
                        value: m.value,
                    });
                }
                nietzsche_proto::db::filter::Condition::Range(r) => {
                    let (gte, lte) = range_bounds_f64(&r);
                    complex_filters.push(nietzsche_core::FilterExpr::Range {
                        key: r.key,
                        gte,
                        lte,
                    });
    ...
    let params = nietzsche_core::SearchParams {
...
fn build_graph_node(
    col: &Arc<dyn nietzsche_core::Collection>,
    id: u32,
    layer: usize,
) -> GraphNode {
...
fn matches_filter_exprs(
    ...
    for expr in complex_filters {
        match expr {
            nietzsche_core::FilterExpr::Match { key, value } => match metadata.get(key) {
                Some(actual) if actual == value => {}
                _ => return false,
            },
            nietzsche_core::FilterExpr::Range { key, gte, lte } => {
            ...
            _ => { /* And/Or/Not/Contains/Exists: not implemented for base filter */ }
        }
    }
    true
}
...
fn parse_vacuum_filter(
    filter: Option<nietzsche_proto::db::VacuumFilterQuery>,
) -> Result<Option<nietzsche_core::VacuumFilterQuery>, Status> {
    let Some(filter) = filter else {
        return Ok(None);
    };
    let op = match filter.op.to_lowercase().as_str() {
        "lt" => nietzsche_core::VacuumFilterOp::Lt,
        "lte" => nietzsche_core::VacuumFilterOp::Lte,
        "gt" => nietzsche_core::VacuumFilterOp::Gt,
        "gte" => nietzsche_core::VacuumFilterOp::Gte,
        "eq" => nietzsche_core::VacuumFilterOp::Eq,
        "ne" => nietzsche_core::VacuumFilterOp::Ne,
        other => {
    ...
    Ok(Some(nietzsche_core::VacuumFilterQuery {
        key: filter.key,
        op,
        value: filter.value,
    }))
...
pub struct NietzscheBaseService {
...
#[tonic::async_trait]
impl Database for NietzscheBaseService {
    // --- Collection Management ---

    async fn create_collection(
        &self,
        request: Request<CreateCollectionRequest>,
    ) -> Result<Response<nietzsche_proto::db::StatusResponse>, Status> {
        ...
        {
            Ok(()) => Ok(Response::new(
                nietzsche_proto::db::StatusResponse {
    ...
    async fn insert(
        ...
        if let Some(col) = self.manager.get(&user_id, &col_name).await {
            ...
            // Durability mapping
            let durability = match nietzsche_proto::db::DurabilityLevel::try_from(
                req.durability,
            )
            .ok()
            {
                Some(nietzsche_proto::db::DurabilityLevel::Strict) => {
                    nietzsche_core::Durability::Strict
                }
                Some(nietzsche_proto::db::DurabilityLevel::Async) => {
                    nietzsche_core::Durability::Async
                }
                Some(nietzsche_proto::db::DurabilityLevel::Batch) => {
                    nietzsche_core::Durability::Batch
                }
                _ => nietzsche_core::Durability::Default,
            };
    ...
    async fn replicate(
        &self,
        request: Request<nietzsche_proto::db::ReplicationRequest>,
    ...
    async fn trigger_snapshot(
        &self,
        _request: Request<nietzsche_proto::db::Empty>,
    ) -> Result<Response<nietzsche_proto::db::StatusResponse>, Status> {
    ...
    async fn rebuild_index(
        &self,
        request: Request<nietzsche_proto::db::RebuildIndexRequest>,
    ) -> Result<Response<nietzsche_proto::db::StatusResponse>, Status> {
        let user_id = get_user_id(&request);
        let req = request.into_inner();
            ...
            loop {
                let log = match rx.recv().await {
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(skipped)) => {
                        eprintln!("Event stream lagged, skipped {skipped} messages");
                        continue;
                    }
                ...
                let event = match log.operation {
                    Some(replication_log::Operation::Insert(op)) => {
                        ...
                        EventMessage {
                            r#type: ty,
                            payload: Some(nietzsche_proto::db::event_message::Payload::VectorInserted(
                                VectorInsertedEvent {
                                    id: op.id,
                    ...
                    Some(replication_log::Operation::Delete(op)) => {
                        ...
                        EventMessage {
                            r#type: ty,
                            payload: Some(
                                nietzsche_proto::db::event_message::Payload::VectorDeleted(
                                    VectorDeletedEvent {
                                        id: op.id,
                                        collection: log.collection.clone(),
                                        logical_clock: log.logical_clock,
...
async fn start_server(args: Args) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let dim_str = std::env::var("NDB_DIMENSION").unwrap_or("1024".to_string());
    let dim: u32 = dim_str.parse().unwrap_or(1024);

    // Support HS_METRIC (new) and HS_DISTANCE_METRIC (legacy)
    let metric = std::env::var("NDB_METRIC")
        .or_else(|_| std::env::var("NDB_DISTANCE_METRIC"))
        .unwrap_or("poincare".to_string())
        .to_lowercase();

    println!("🚀 Booting NietzscheDB | Dim: {dim} | Metric: {metric}");
    ...
    if args.role == "follower" {
        if let Some(leader) = args.leader.clone() {
            ...
            let api_key_for_client = std::env::var("NDB_API_KEY").ok();
                ...
                loop {
                        ...
                        .await
                    {
                        Ok(channel) => {
                            ...
                            let req = nietzsche_proto::db::ReplicationRequest {
                                                ...
                                                Some(replication_log::Operation::Insert(op)) => {
                                                        ...
                                                        mgr.get_internal(col_name).await
                                                    {
                                                        if let Err(e) = col.insert(
                                                            ...
                                                            log.logical_clock,
                                                            nietzsche_core::Durability::Default,
                                                        ).await {
    ...
    let vectorizer: Option<Arc<dyn Vectorizer>> = {
        ...
        let enabled = std::env::var("NDB_EMBED")
            .unwrap_or_else(|_| "true".to_string())
            .to_lowercase()
            == "true";

        if enabled {
            let provider_str = std::env::var("NDB_EMBED_PROVIDER")
                .unwrap_or_else(|_| "local".to_string())
                .to_lowercase();

            let metric_str = std::env::var("NDB_METRIC")
                .or_else(|_| std::env::var("NDB_DISTANCE_METRIC"))
                .unwrap_or("poincare".to_string())
                .to_lowercase();
            ...
            if provider_str == "local" && metric != Metric::Poincare {
                ...
                eprintln!("   Provider 'local' (Hyperbolic ONNX) requires NDB_METRIC='poincare'.");
                eprintln!("   Found NDB_METRIC='{metric_str}'.");
                eprintln!("🛑 Embedding Service Disabled to prevent mathematical errors.");
                None
            } else if provider_str == "local" {
                let model_path = std::env::var("NDB_MODEL_PATH").ok();
                let tok_path = std::env::var("NDB_TOKENIZER_PATH").ok();

                if let (Some(m), Some(t)) = (model_path, tok_path) {
                    println!("🧠 Loading Local Embedding Model: {m}");
                    let dim: usize = std::env::var("NDB_EMBED_DIM")
                    ...
                    println!(
                        "⚠️ Embedding Service needs NDB_MODEL_PATH for 'local' provider."
                    );
                    None
                }
            } else {
                ...
                if let Ok(provider) = ApiProvider::from_str(&provider_str) {
                    let api_key = std::env::var("NDB_API_KEY_EMBED")
                        .or_else(|_| std::env::var("OPENAI_API_KEY"))
                        .unwrap_or_default();

                    let model = std::env::var("NDB_EMBED_MODEL")
                        .unwrap_or("text-embedding-3-small".to_string());

                    let base_url = std::env::var("NDB_API_BASE").ok();
            ...
            println!("🛑 Embedding Service Disabled (NDB_EMBED=false)");
            None
        }
    };
    ...
    let embedding_info = {
        #[cfg(feature = "embed")]
        {
            if let Some(v) = &vectorizer {
                let provider =
                    std::env::var("NDB_EMBED_PROVIDER").unwrap_or("local".to_string());
                let model =
                    std::env::var("NDB_EMBED_MODEL").unwrap_or("default".to_string());
                ...
                let enabled_flag = std::env::var("NDB_EMBED")
    ...
    let service = NietzscheBaseService {
        manager,
        replication_tx,
        role: args.role,
        #[cfg(feature = "embed")]
    ...
    println!("NietzscheDB listening on {addr}");
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    println!("[N] NietzscheDB Server v2.0.0");
    nietzsche_core::check_simd();
