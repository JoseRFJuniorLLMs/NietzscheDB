...
impl Node {
    fn spawn(grpc: u16, http: u16, role: &str, leader: Option<&str>, clean: bool) -> Self {
        let server_path = std::env::current_dir()
            .unwrap()
            .join("target/release/nietzsche-baseserver");
        ...
        // Pass API Key
        cmd.env("NDB_API_KEY", "I_LOVE_NIETZSCHEDB");
        cmd.env("NDB_WAL_SYNC_MODE", "strict");
        cmd.env("NDB_SNAPSHOT_INTERVAL_SEC", "1");
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let status = Command::new("cargo")
        .args(["build", "--release", "-p", "nietzsche-baseserver"])
        .status()?;
